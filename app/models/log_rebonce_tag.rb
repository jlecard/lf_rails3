class LogRebonceTag < ActiveRecord::Base
  
  def self.total(unit = "day", date_from_str = nil, date_to_str = nil, profil = nil, order = "time",  page = 1, max = 50)
    select_clause = "t.label tag_label, count(l.tag_id) total"
    from_clause = ", tags t" 
    where_clause = "t.id = l.tag_id"  
    group_by_clause = "l.tag_id"
  ...
  def self.by_notice(unit = "day", date_from_str = nil, date_to_str = nil, profil = nil, order = "time",  page = 1, max = 50)
    select_clause = "t.label tag_label, count(l.tag_id) total"
    from_clause = ", tags t" 
    where_clause = "t.id = l.tag_id and l.object_type = #{ENUM_NOTICE}"  
    group_by_clause = "l.tag_id"
    return self.generic_request(select_clause, from_clause, where_clause, group_by_clause, unit, date_from_str, date_to_str, profil, order, page, max)
  end
  
  def self.by_liste(unit = "day", date_from_str = nil, date_to_str = nil, profil = nil, order = "time",  page = 1, max = 50)
    select_clause = "t.label tag_label, count(l.tag_id) total"
    from_clause = ", tags t" 
    where_clause = "t.id = l.tag_id and l.object_type = #{ENUM_LIST}"  
    group_by_clause = "l.tag_id"
    return self.generic_request(select_clause, from_clause, where_clause, group_by_clause, unit, date_from_str, date_to_str, profil, order, page, max)
  end
  ...
  def self.generic_request(select_clause, from_clause, where_clause, group_by_clause, unit = "day", date_from_str = nil, date_to_str = nil, profil = nil, order = "time", page = 1, max = 50)
    ...
    requete += " from log_rebonce_tags l "
    ...
    res = LogRebonceTag.find_by_sql(requete)
    
    total = 0
    count = LogRebonceTag.find_by_sql("SELECT FOUND_ROWS() as total")
