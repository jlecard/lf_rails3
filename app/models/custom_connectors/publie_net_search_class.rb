...
class PublienetSearchClass < ActionController::Base
  ...
  def self.SearchCollection(_collect, _qtype, _qstring, _start, _max, _qoperator, _last_id, job_id = -1, infos_user=nil, options=nil, _session_id=nil, _action_type=nil, _data = nil, _bool_obj=true)
    ...
    _lrecord = RetrievePublieNet_Single(_last_id, _query, _qtype, _qstring, _qoperator, _type, _coll_list, _alias, _group, _vendor_url, _is_parent, _col_name,_max.to_i, _collect.filter_query, infos_user, options, _availability)
    ...
    logger.warn("#STAT# [#{self.name}] base: #{_collect.name}[#{_coll_list}] total: " + sprintf( "%.2f",(Time.now().to_f - _sTime)).to_s) if LOG_STATS
  ...
  def self.RetrievePublieNet_Single(_search_id, _query, _qtype, _qstring, _qoperator, _type, _coll_list, _alias, _group, _vendor_url,  _is_parent, _collection_name,  _max, _filter_query=nil, infos_user=nil, options=nil, _availability=nil)
    ...
    return  RetrievePublieNet(_search_id, _query, _qtype, _qstring, _qoperator, htype, _coll_list, halias, hgroup, hvendor, hparent, hcolname, _max, _filter_query, infos_user, options, _availability)
  end
  
  def self.RetrievePublieNet(_search_id, _query, _qtype, _qstring, _qoperator, _type, _coll_list, _alias, _group, _vendor_url,  _is_parent, _collection_name, _max,filter_query=nil, infos_user=nil,  options=nil, _availability=nil)
    _x = 0
    _y = 0
    _oldset = ""
    _newset = ""
    ...
    logger.warn("#STAT# [#{self.name}] base: [#{_coll_list}] search: " + sprintf( "%.2f",(Time.now().to_f - _sTime)).to_s) if LOG_STATS
    ...
    begin
      ...
      _results.each { |_row|
        if _row.oai_identifier.to_s == idDoc.to_s
          ...
          record.isbn = ""
          record.doi = ""
          record.direct_url = chkString(_row.osu_linking)
          record.static_url = ""
          record.callnum = ""
          record.page = ""
          record.number = ""
