...
class FactivaSearchClass < ActionController::Base
  ...
  def search
    ...
    if proxy?
      logger.debug("[FactivaSeachClass] [search] proxy set = #{@proxy_host} : #{@proxy_port}")
      @http = Net::HTTP::Proxy(@proxy_host,@proxy_port.to_i).new('developer.beta.factiva.com', 80)
    else
      logger.debug("[FactivaSeachClass] [search] NO PROXY")
              ...
              <soap:Envelope xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance' xmlns:xsd='http://www.w3.org/2001/XMLSchema' xmlns:soap='http://schemas.xmlsoap.org/soap/envelope/'>
                <soap:Body>
                  <PerformContentSearch xmlns='urn:factiva:developer:v3_0:parsers'>
                    ...
                    <searchString>#{@pkeyword}</searchString>
      ...
      begin
        ...
        while i < loops
          ...
          threads << Thread.new(i, context) do |i, context|
            ...
            result = continue_search(context, Thread.current["mycount"])
  ...
  def parse_results(doc)
    begin
      ...
      logger.debug("[FactivaSearchClass][parse_results] error #{e.backtrace}")
    end
