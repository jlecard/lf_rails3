...
class EmconsulteSearchClass < ActionController::Base
  ...
  @total_hits = 0
  def search
    @pkeywords = @query_string.join(" ")
    logger.debug("Entered EmConsulteSearchClass")
    query = CGI::escape(@pkeywords)
    ...
    begin
      ...
      parse_em_consulte(xml_result_set.body)
    rescue => e
    ...
    nodes.each do |item|
      ...
      begin
        ...
        if !material_type.nil?
          DocumentType.save_document_type(material_type, @collection.id)
