class LogSaveNotice < ActiveRecord::Base
  
  def self.notice_save_total(unit = "day", date_from_str = nil, date_to_str = nil, profil = nil, order = "time",  page = 1, max = 50)
    select_clause = "profil, count(saveIn) total"
    from_clause = "" 
    where_clause = "1=1" 
  ...
  def self.notice_save_in_list(unit = "day", date_from_str = nil, date_to_str = nil, profil = nil, order = "time",  page = 1, max = 50)
    select_clause = "profil, count(saveIn) total"
    from_clause = "" 
    where_clause = "saveIn='list'" 
    group_by_clause = "profil"
    return self.generic_request(select_clause, from_clause, where_clause, group_by_clause, unit, date_from_str, date_to_str, profil, order, page, max)
  end
  
  def self.notice_save_mydoc(unit = "day", date_from_str = nil, date_to_str = nil, profil = nil, order = "time",  page = 1, max = 50)
    select_clause = "profil, count(saveIn) total"
    from_clause = "" 
    where_clause = "saveIn='mydoc'"  
    group_by_clause = "profil"
    return self.generic_request(select_clause, from_clause, where_clause, group_by_clause, unit, date_from_str, date_to_str, profil, order, page, max)
  end
  ...
  def self.generic_request(select_clause, from_clause, where_clause, group_by_clause, unit = "day", date_from_str = nil, date_to_str = nil, profil = nil, order = "time", page = 1, max = 50)
    ...
    requete += " from log_save_notices l "
    ...
    res = LogSaveNotice.find_by_sql(requete)
    
    total = 0
    count = LogSaveNotice.find_by_sql("SELECT FOUND_ROWS() as total")
