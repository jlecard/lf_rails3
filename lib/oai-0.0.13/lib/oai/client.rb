...
module OAI
  ...
  class Client
    ...
    def proxy(host, port)
      ...
      @use_proxy = true
      if !@logger.nil?
        @logger.info("[ClientOAI] use proxy : #{@host} : #{@port}")
      end
    end
    ...
    def do_request(verb, opts = nil)
      ...
      if @parser == 'libxml' or @parser == 'nokogiri'
    ...
    def load_document(xml)
      case @parser
        when 'libxml'
        begin
          return XML::Parser.string(xml).parse
        rescue XML::Parser::ParseError => e
          raise OAI::Exception, 'response not well formed XML: : #{e} xml: #{xml}', caller
        rescue => e
          raise e
        end
    ...
    def get(uri)
      ...
      if !@logger.nil?
        @logger.debug("[ClientOAI] uri : #{uri}")
      end
      
      if @use_proxy
        response = Net::HTTP::Proxy(@host.to_s, @port.to_s).get_response(uri)
      else
